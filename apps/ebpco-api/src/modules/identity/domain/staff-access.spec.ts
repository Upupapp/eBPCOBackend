...
const PERMIT_TYPES = ['Building Permit', 'Fencing Permit', 'Demolition Permit', 'Electrical Permit'];
...
describe('the forms allow-list fails closed', () => {
  ...
  it('treats an unassigned account as no access, not all access', () => {
    ...
    expect(mayWorkOn(NO_ACCESS, 'Building Permit')).toBe(false);
  });
  ...
  it('keeps a retired permit type inert rather than erroring', () => {
    ...
    expect(mayWorkOn(granted, 'Building Permit')).toBe(false);
  });
  ...
  it('is case- and whitespace-exact, because these are keys not labels', () => {
    ...
    // migration 033 (D-10) they are the same string, and since migration 047
    // consolidated the three Building Permit sub-types into one, what is left
    // is the sharper case: a RETIRED sub-type key and the wrong casing of the
    // live one. Both are what a stale client or a hand-edited config would send.
    const granted = access('view-edit', ['Building Permit']);

    expect(mayWorkOn(granted, 'building permit')).toBe(false);
    expect(mayWorkOn(granted, 'Building Permit ')).toBe(false);
    // The retired sub-type key -- exactly what a client that has not picked
    // up 047 yet would still be sending.
...
describe('level and forms answer different questions', () => {
  ...
  it.each(table)('level=$level forms=$forms.length', ({ level, forms, act, see }) => {
    ...
    expect(mayWorkOn(granted, 'Building Permit')).toBe(see);
  });
});
