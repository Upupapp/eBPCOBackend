...
| `BODY_LIMIT_BYTES` | `1048576` | An unbounded body is a denial-of-service surface. Applies to every route **except** the ones that carry a file (below). |
| `UPLOAD_BODY_LIMIT_BYTES` | `29360128` | Only the routes marked `@UploadRoute()` (document upload and replacement, profile photo). Sized for the documents service's own 20MB per-file cap once base64 adds a third. **Too low and a scanned plan or a phone photo is refused with a bare 413 that a phone, still sending, sees only as a dropped connection** — the ceiling was ~750KB while this shared the 1MB above. `GET /limits` publishes the resulting file ceiling, so the clients follow it without a release. |
| `UPLOAD_TIMEOUT_MS` | `300000` | Only the upload routes. The request budget runs from the first byte, so it includes receiving the file; a 20MB file over a phone's mobile data takes minutes, and the 20s JSON budget would cut it off mid-send. |
| `RATE_LIMIT_MAX` | `300` | — |
