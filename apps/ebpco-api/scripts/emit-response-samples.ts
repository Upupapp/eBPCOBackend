...
import { stabilise } from '../src/common/evidence/stabilise';

/**
 * Where the evidence is written.
 *
 * Inside THIS repository by default. It used to default to
 * `../ebpco-contract/reconciliation/`, which meant regenerating the backend's
 * own evidence wrote into a sibling repository owned by a different lane —
 * a script that edits somebody else's checkout as a side effect of running
 * `npm run emit:samples`.
 *
 * The path is still an argument, so handing the files over is one command.
 */
const outputDirectory = process.argv[2] ?? resolve(__dirname, '../contract');
const target = resolve(outputDirectory, 'response-samples.json');
...
async function main(): Promise<void> {
  ...
  // The real route table, collected as Fastify registers each route. This is
  // what makes "declared in the contract" and "implemented by the server" two
  // separable claims — a path can be in the OpenAPI document and reach nothing,
  // which is what every client-facing endpoint was until TAB 13.
  const routes: string[] = [];
  ...
  // The write path. Recorded because these are the responses the offline queue
  ...
  const exportRequestId = (samples['me.export.request'] as { body: { requestId: string } })
    .body.requestId;
  await record('me.export.status', 'GET', `/me/export/${exportRequestId}`, applicantToken);
  await record('problem.exportNotReady', 'GET',
    `/me/export/${exportRequestId}/content`, applicantToken);
  ...
  writeFileSync(
    resolve(outputDirectory, 'route-table.json'),
    `${JSON.stringify({
      _comment:
        'GENERATED by ebpco-api/scripts/emit-response-samples.ts from the real Fastify route table. '
        + 'The contract repository checks that every path a client calls is not merely DECLARED but '
        + 'actually reachable — a path can be in the OpenAPI document and reach nothing, which is what '
        + 'every client-facing endpoint was before TAB 13.',
      contractVersion: '0.1.0',
      routes: [...routes].sort(),
    }, null, 2)}\n`,
    'utf8',
  );
  process.stdout.write(`wrote the route table (${routes.length} routes)\n`);
  ...
  writeFileSync(target, `${JSON.stringify(stabilise(document), null, 2)}\n`, 'utf8');
